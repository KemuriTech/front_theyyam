...
export default {
  ...
  async mounted() {
    ...
    if (this.eventData.length > 0) {
      const latSum = this.eventData.reduce((total, event) => total + parseFloat(event.ext_11), 0);
      const lngSum = this.eventData.reduce((total, event) => total + parseFloat(event.ext_12), 0);
      const avgLat = latSum / this.eventData.length;
      const avgLng = lngSum / this.eventData.length;
