        ...
        <TabGroup v-if="mediaArr.length" as="div" class="flex flex-col-reverse">
          ...
          <div class="hidden mt-6 w-full max-w-2xl mx-auto sm:block lg:max-w-none">
            <TabList class="grid grid-cols-4 gap-6">
              <Tab v-for="(media, id) in mediaArr" :key="id" v-slot="{ selected }"
                class="relative h-24 bg-white rounded-md flex items-center justify-center text-sm font-medium uppercase text-gray-900 cursor-pointer hover:bg-gray-50 focus:outline-none focus:ring focus:ring-offset-4 focus:ring-opacity-50">
                <span class="sr-only">
                  {{ media.type }}
                </span>
                <span class="absolute inset-0 rounded-md overflow-hidden">
                  <img :src="getImage({ type: media.type, url: media.url })" alt=""
          ...
          <TabPanels class="w-full aspect-w-1 aspect-h-1">
            <TabPanel v-for="(media, id) in mediaArr" :key="id">
              <img v-if="media.type === MEDIA_TYPE.IMAGE" :src="media.url" alt="Temple"
                class="w-full h-[21rem] object-center object-cover sm:rounded-lg" />
              <div v-else-if="media.type === MEDIA_TYPE.YT_VIDEO" class='w-full h-[21rem]'>
                <iframe :src="$ytVideo.getYTVideoUrl(media.url, 'mute=0&modestbranding=1&autoplay=1')"
                  allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
                  allowfullscreen class="w-full h-full sm:rounded-lg" frameborder="0"></iframe>
              </div>
            </TabPanel>
          </TabPanels>
        </TabGroup>
            ...
            <div class="border-t divide-y divide-gray-200">
                ...
                <h3>
                  <DisclosureButton class="group relative w-full py-6 flex justify-between items-center text-left">
                    ...
                    <span class="ml-6 flex items-center">
                      <PlusIcon v-if="!open" aria-hidden="true"
              ...
              <Disclosure v-slot="{ open }" as="div">
                ...
                <DisclosurePanel as="div" class="pb-6 prose prose-sm">
                  ...
                  <a :href="`https://www.google.com/maps/search/?api=1&query=${eventData.ext_11}%2C${eventData.ext_12}`"
                    class="py-2.5 text-center no-underline text-gray-600 font-normal" target="_blank">
                    Open in Google Maps
              ...
              <Disclosure v-slot="{ open }" as="div">
                ...
                <DisclosurePanel as="div" class="pb-6 prose prose-sm">
                  <div class="grid grid-cols-2">
                    <ContactInfo v-for="info in contactInfos" :key="info.id" :contact-info="info" />
                  </div>
                </DisclosurePanel>
              </Disclosure>
            </div>
